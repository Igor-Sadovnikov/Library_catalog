...
class AddReviewForm(FlaskForm): # форма добавления отзыва
    review = StringField('Поле для вашего отзыва:', validators=[DataRequired()])
...
class Reviews(SqlAlchemyBase): # модель отзыва
    __tablename__ = 'reviews'

    id = sqlalchemy.Column(sqlalchemy.Integer, 
                        primary_key=True, autoincrement=True)
    id_book = sqlalchemy.Column(sqlalchemy.Integer, nullable=True)
    id_author = sqlalchemy.Column(sqlalchemy.Integer, nullable=True)
    text = sqlalchemy.Column(sqlalchemy.String, nullable=True)
    date_of_review = sqlalchemy.Column(sqlalchemy.Text, nullable=True)
...
@app.route('/list_of_reviews/<int:book_id>')
def list_of_reviews(book_id): # отображение списка отзывов
    session = db_session.create_session()
    current_reviews, out_reviews = [], []
    for elem in session.query(Reviews).filter(Reviews.id_book == book_id):
        current_reviews.append(elem)
    author_book, name_book = '', ''
    for elem in current_reviews:
        out_review = {}
        out_review['author_of_review'] = session.query(User).filter(User.id == elem.id_author).first().name
        out_review['author_of_review'] += ' ' + session.query(User).filter(User.id == elem.id_author).first().surname
        out_review['review_text'] = elem.text
        out_review['author_of_book'] = session.query(Books).filter(Books.id == book_id).first().author
        out_review['name_of_book'] = session.query(Books).filter(Books.id == book_id).first().name
        out_review['date'] = elem.date_of_review
        out_reviews.append(out_review)
        author_book = out_review['author_of_book']
        name_book = out_review['name_of_book']
    return render_template("list_of_reviews.html",
                           reviews=out_reviews, book_id=book_id,
...
def add_review(book_id): # форма добавления отзыва
    form = AddReviewForm()
    if form.validate_on_submit():
        db_sess = db_session.create_session()
        ...
        new_review = Reviews(
            ...
            date_of_review = date.strftime("%d.%m.%Y")
        )
